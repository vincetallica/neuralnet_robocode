...
public class XOR_BP
{
    ...
    private static int NUM_EPOCHS = 0;
    private static final double LR_IH = 0.2;      // Learning rate, input to hidden weights.
    private static final double LR_HO = 0.2;     // Learning rate, hidden to output weights.

    private static double hiddenNeuron[] = new double[NUM_HIDDEN];    // Hidden node outputs.
    ...
    private static double RMSerror = 1.0;    // Root Mean Squared error.
    ...
    private static void algorithm()
    {
        ...
        // Train the network.
        while(RMSerror > 0.2) //num of iterations
        {
            ...
            System.out.println("epoch = " + NUM_EPOCHS + " RMS Error = " + RMSerror);
    ...
    private static void calcNet(final int patNum)
    {
        ...
        for(int i = 0; i < NUM_HIDDEN; i++)
        {
            ...
            {
                hiddenNeuron[i] += (trainInputs[patNum][j] * weightsIH[j][i]); //sum
            }
            hiddenNeuron[i] = sigmoid(hiddenNeuron[i]); //where sigmoidal (activation function is)
        } // i
    ...
    private static void WeightChangesHO()
    {
        ...
        {
            double weightChange = LR_HO * errThisPat * hiddenNeuron[k];
    ...
    private static void WeightChangesIH(final int patNum)
    {
        ...
        for(int i = 0; i < NUM_HIDDEN; i++)
        {
            ...
            {
                //double x = 1 - Math.pow(hiddenNeuron[i], 2); //for tanh()
                double x = hiddenNeuron[i]*(1-hiddenNeuron[i]);

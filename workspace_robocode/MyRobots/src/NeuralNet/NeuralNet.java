...
public class NeuralNet implements NeuralNetInterface {
	...
	public double train(final double[] X, final double argValue) {
		...
		for (int i = 0; i < NUM_HIDDEN; i++) {
			for (int j = 0; j < NUM_INPUTS; j++) {
				...
				final double deltaweight = LR * errSig * x * (weightsHO[i]) * X[j];
	...
	public void load(final String argFileName) throws IOException {
		// TODO Auto-generated method stub
		for (int j = 0; j < NUM_HIDDEN; j++) {
			System.out.println("WOH["+j+"]: " + weightsHO[j]);
			for (int i = 0; i < NUM_INPUTS; i++) {
				System.out.println("WIH["+i+"]["+j+"]: " + weightsIH[i][j]);
				if (i == NUM_INPUTS-1) {
					System.out.println("W_input_bias["+i+"]["+j+"]: " + weightsIH[i][j]);
				}
			}
		}
		System.out.println("W_bias_output: " + weightsBO[0]);
	}
}
